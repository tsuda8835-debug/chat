...
3. 左の **文書を追加** から、テキストを含む 10 MB 以下の PDF、TXT、または Markdown を登録できます。
4. RAG をオフにすると、ローカル文書を参照しない通常の会話になります。
5. 左の **履歴** で会話を新規作成、再開、削除できます。履歴はリロードやローカルサーバーの再起動後も残ります。

ルートの `knowledge.txt` には、問い合わせ、リリース、セキュリティ、PDF の扱いに関する日本語のサンプルを収録しています。内容を書き換えると、次回プロセス起動後の RAG ナレッジになります。`demo/` には発表用の PDF、Markdown、TXT を各 5 ファイル収録しています。

## RAG の構成

```text
knowledge.txt / PDF / TXT / Markdown upload
           │
      text extraction
           │
   chunking + embedding
...
- PDF は `pdf-parse`、TXT と Markdown はブラウザ標準の `file.text()` で読み込み、同じベクトルストアに追加します。
- 埋め込みは OpenAI を優先し、未設定なら Google を使います。プロセス内では単一の埋め込みモデルを一貫して使います。
- API はメッセージ件数・文字数、文書種別、サイズ、抽出テキストを検証します。PDF の先頭シグネチャとテキスト文書のバイナリ内容を確認し、ファイル名は表示用に無害化します。
- 会話とメッセージは `data/chat.db` の SQLite に保存します。`data/` は Git 管理対象外です。

## 重要な制限

これはデモおよびローカル用途向けの実装です。会話履歴はローカル SQLite に保存されますが、アップロード文書とベクトルはインメモリのため、サーバー再起動、ホットリロード、サーバーレスのインスタンス切り替えで失われます。SQLite のローカルファイルも Vercel などのサーバーレス環境では永続化されません。また、アップロード文書は同一プロセスの利用者全員の検索対象になるため、機密情報をアップロードしないでください。本番環境では認可された永続ストレージとテナント分離された会話・ベクトルデータベースに置き換えてください。
